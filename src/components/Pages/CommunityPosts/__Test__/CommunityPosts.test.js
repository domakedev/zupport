...
import {softNumber, getPostTime} from "../../../../controller/CommunityPostCtr/utilities";
...
describe('welcome you to a community posts page',()=>{
  ...
  test('It should render the welcome to a community posts page',()=>{    
    ...
    expect(getText(/Bienvenid@ a tu comunidad Javascript/i)).toBeInTheDocument();  
  });

  test('It should render the statement of the community top helpers',()=>{
     expect(getText(/Conoce a l@s 5 top helpers de tu comunidad/i)).toBeInTheDocument();  
  });
});
...
describe('Comment actions and like posts',()=>{
  test('It should render the option to like a post',()=>{
    const buttonElement = screen.getAllByRole('button',{name:/Te apoyo/i})[0];
    userEvent.click(buttonElement)    
    expect(buttonElement).toBeInTheDocument();
  });
  test('It should render the option to comment on a post',()=>{
    const buttonElement = screen.getAllByRole('button',{name:/Responder/i})[0];
    userEvent.click(buttonElement)
    expect(buttonElement).toBeInTheDocument();
  });
})
